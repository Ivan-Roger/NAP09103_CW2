...
var EncryptedKey = function(value) {
  console.log("PrivK: Constructor >",value);
  var obj = {};
  obj._value = value;
  obj._encrypted = true;
  obj.decrypt = function (passphrase, callback) {
    console.log("PrivK: decrypt() ...");
    if (obj.isDecrypted()) {
    ...
    openpgp.decryptKey({privateKey: obj._value, passphrase: passphrase}).then(function (unlocked) {
      ...
      obj._decrypted = true;
      console.log("PrivK: Decrypted !");
      if (callback) callback(this);
    });
    console.log("PrivK: Decrypting ...");
  }
...
socket.on('serverKey', function(data) {
  ...
  openpgp.encrypt(options).then(function(ciphertext) {
    ...
    socket.emit('encMessage', {user: 'User_000', message: encryptedMsg});
  });
});

socket.on('encMessage', function(data) {
  console.log("Decrypting message from ("+data.user+")");
  ...
  openpgp.decrypt(options).then(function(plaintext) {
    ...
    msg.querySelector(".app-chatMsgUser .app-msgUserName").innerHTML = data.user;
